...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiError> handleBffException(BffServiceException ex, HttpServletRequest request) {
		...
		// 2. Optimized Duplicate Key Message else
		if (error.getMessage() != null && error.getMessage().contains("duplicate key value")) {
			String rawMsg = error.getMessage();
			if (rawMsg.contains("property_id")) {
				String id = rawMsg.substring(rawMsg.indexOf("(property_id)=(") + 15, rawMsg.indexOf(") already"));
				error.setMessage("Property ID " + id + " already exists.");
			}
		}
